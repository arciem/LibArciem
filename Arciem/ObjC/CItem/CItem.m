    ...
    if([dummyValue isKindOfClass:[NSString class]]) {
        ...
        for(NSUInteger i = 0; i < originalDummyValue.length; i++) {
            ...
            if([c isEqualToString:@"#"]) {
                NSString *s = [NSString stringWithFormat:@"%d", (int)(random_range(0, 10))];
...
- (NSString*)jsonRepresentation {
	...
	NSError *error = nil;
	NSData *outData = [NSJSONSerialization dataWithJSONObject:outDict options:(NSJSONWritingOptions)0 error:&error];
