...
- (NSDate*)dateForString:(NSString*)str {
    ...
    if([str isEqualToString:@"now"]) {
        ...
        NSDateComponents* comps = [self.calendar components:(NSCalendarUnit)(NSYearCalendarUnit | NSMonthCalendarUnit) fromDate:[NSDate date]];
...
- (void)setup {
	...
	if([self.maxDate isKindOfClass:[NSString class]]) {
		...
		if(comps != nil) {
			if(self.minDate != nil) {
				date = [self.calendar dateByAddingComponents:comps toDate:self.minDate options:(NSCalendarOptions)0];
			}
		}
