...
- (NSString*)stringByReplacingTemplatesWithReplacements:(NSDictionary*)replacementsDict {
    ...
    if(regex == nil) {
        regex = [[NSRegularExpression alloc] initWithPattern:@"\\{(.*?)\\}" options:(NSRegularExpressionOptions)0 error:&error];
    }
    NSInteger offset = 0;
    for(NSTextCheckingResult* result in [regex matchesInString:self options:(NSMatchingOptions)0 range:NSMakeRange(0, self.length)]) {
...
- (NSArray*)capturesFromMatchesOfRegularExpression:(NSRegularExpression*)regex stopAfterFirstMatch:(BOOL)stopAfterFirstMatch stopAfterFirstCapture:(BOOL)stopAfterFirstCapture {
    ...
    [regex enumerateMatchesInString:self options:(NSMatchingOptions)0 range:NSMakeRange(0, self.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
...
- (BOOL)matchesRegularExpression:(NSRegularExpression *)regex {
    NSRange r = [regex rangeOfFirstMatchInString:self options:(NSMatchingOptions)0 range:NSMakeRange(0, self.length)];
...
- (NSAttributedString *)stringByReplacingTemplatesWithReplacements:(NSDictionary*)replacementsDict attributes:(NSDictionary *)attributesDict {
    ...
    NSInteger offset = 0;
    for(NSTextCheckingResult* result in [regex matchesInString:self.string options:(NSMatchingOptions)0 range:NSMakeRange(0, self.length)]) {
        NSRange resultRange = result.range;
        resultRange.location += offset;
