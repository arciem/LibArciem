...
area area::set_x_min(double x, bool flexible) const
{
	if(flexible) {
		double xMax = x_max();
		return area(min(x, xMax), y_min(), xMax, y_max(), 0);
	} else {
...
area area::set_y_min(double y, bool flexible) const
{
	if(flexible) {
		double yMax = y_max();
		return area(x_min(), min(y, yMax), x_max(), yMax, 0);
	} else {
...
area area::set_x_max(double x, bool flexible) const
{
	if(flexible) {
		double xMin = x_min();
		return area(xMin, y_min(), max(x, xMin), y_max(), 0);
	} else {
...
area area::set_y_max(double y, bool flexible) const
{
	if(flexible) {
		double yMin = y_min();
		return area(x_min(), yMin, x_max(), max(y, yMin), 0);
	} else {
