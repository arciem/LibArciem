...
- (instancetype)initWithAnnotation:(id<MKAnnotation>)annotation reuseIdentifier:(NSString *)reuseIdentifier
{
	if(self = [super initWithAnnotation:annotation reuseIdentifier:reuseIdentifier]) {
		...
		[self addObserver:self forKeyPath:@"annotation" options:(NSKeyValueObservingOptions)(NSKeyValueObservingOptionInitial | NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld) context:kAnnotationChangedContext];
	}
...
//		CLogDebug(nil, @"%@ annotation changed:%@", self, change);
		...
		if(oldAnnotation != newAnnotation) {
			...
			if(!IsNull(newAnnotation)) {
				[newAnnotation addObserver:self forKeyPath:@"coordinate" options:(NSKeyValueObservingOptions)(NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial) context:kAnnotationCoordinateChangedContext];
			}
		}
