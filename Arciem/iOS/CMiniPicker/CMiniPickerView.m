...
- (void)syncToScroll
{
    ...
    CGFloat clampedOffset = clamp(offset, minOffset, maxOffset);
    ...
    for(NSUInteger index = 0; index < self.contentViews.count; index++) {
        ...
        if(clampedOffset >= viewTopOffset && clampedOffset < viewBottomOffset) {
            selectedIndex = index;
            selectedIndexOffsetFraction = map(clampedOffset, viewTopOffset, viewBottomOffset, (CGFloat)(-0.5), (CGFloat)(0.5));
            CGFloat overlayHeight = view.height;
            if(selectedIndexOffsetFraction < 0.0 && index > 0) {
                UIView *previousView = self.contentViews[index - 1];
                overlayHeight = denormalize(-selectedIndexOffsetFraction, overlayHeight, previousView.height);
            } else if(selectedIndexOffsetFraction > 0.0 && index < (self.contentViews.count - 1)) {
                UIView *nextView = self.contentViews[index + 1];
                overlayHeight = denormalize(selectedIndexOffsetFraction, overlayHeight, nextView.height);
            }
    ...
    if(self.lastSelectedIndex != NSNotFound) {
        ...
        } else if(abs((float)indexOffset) >= 2) {
            [self playDetentSound];
        }
    }
...
- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset
{
    ...
    CGFloat clampedTargetOffset = clamp(targetOffset, minOffset, maxOffset);
...
//            selectedIndex = index;
            targetContentOffset->y = denormalize((CGFloat)(0.5), viewTopOffset, viewBottomOffset);
            break;
        }
