...
extern NSString* const kNeedsUpdateItemsNotification;
...
//	self.backgroundColor = [[UIColor blueColor] colorWithAlphaComponent:0.25];
	...
	[self addObserver:self forKeyPath:@"notifier" options:(NSKeyValueObservingOptions)(NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew) context:NULL];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	...
	if(object == self) {
		if([keyPath isEqualToString:@"notifier"]) {
			...
			[newNotifier addObserver:self forKeyPath:@"items" options:(NSKeyValueObservingOptions)(NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial) context:NULL];
