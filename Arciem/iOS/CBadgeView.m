...
- (CGSize)imageOffset {
    ...
    CGFloat ox = fmax((CGFloat)0, imageBounds.origin.x - shadowBounds.origin.x);
    CGFloat oy = fmax((CGFloat)0, imageBounds.origin.y - shadowBounds.origin.y);
    CGSize offset = CGSizeMake(ox, oy);
    return offset;
    //    CGRect shadowBounds = self.shadowBounds;
//    return CGSizeMake(shadowBounds.origin.x, shadowBounds.origin.y);
//    CGFloat ox = fmax(0.0f, -self.shadowOffset.width);
//    CGFloat oy = fmax(0.0f, -self.shadowOffset.height);
...
- (void)drawStrokeWithContext:(CGContextRef)context rect:(CGRect)rect {
    ...
    CGFloat lineWidth = fmax((CGFloat)1, (CGFloat)2 * self.scaleFactor);
