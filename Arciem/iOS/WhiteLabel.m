...
- (NSString*)stringByReplacingTemplatesInString:(NSString*)str
{
    ...
    NSError* error = nil;
    NSRegularExpression* regex = [[NSRegularExpression alloc] initWithPattern:@"\\{(.*?)\\}" options:(NSRegularExpressionOptions)0 error:&error];
    NSInteger offset = 0;
    for(NSTextCheckingResult* result in [regex matchesInString:str options:(NSMatchingOptions)0 range:NSMakeRange(0, str.length)]) {
