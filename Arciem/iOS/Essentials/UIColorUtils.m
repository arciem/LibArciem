...
- (UIColor*)newColorByCircularInterpolatingToColor:(UIColor*)color fraction:(CGFloat)fraction
{
	...
	if([self getHue:&h1 saturation:&s1 brightness:&b1 alpha:&a1]) {
		if([self getHue:&h2 saturation:&s2 brightness:&b2 alpha:&a2]) {
			h = circular_interpolate(fraction, h1, h2);
			s = denormalize(fraction, s1, s2);
			b = denormalize(fraction, b1, b2);
			a = denormalize(fraction, a1, a2);
...
- (UIColor*)newColorByDeepeningFraction:(CGFloat)fraction
{
	...
	CGFloat h, s, b, a;
	if([self getHue:&h saturation:&s brightness:&b alpha:&a]) {
		b = denormalize(fraction, b, (CGFloat)0);
...
- (UIColor*)newColorBySaturatingFraction:(CGFloat)fraction
{
	...
	CGFloat h, s, b, a;
	if([self getHue:&h saturation:&s brightness:&b alpha:&a]) {
		s = denormalize(fraction, s, (CGFloat)1.0);
		color = [UIColor colorWithHue:h saturation:s brightness:b alpha:a];
	}
...
- (UIColor*)newColorByDesaturatingFraction:(CGFloat)fraction
{
	...
	CGFloat h, s, b, a;
	if([self getHue:&h saturation:&s brightness:&b alpha:&a]) {
		s = denormalize(fraction, s, (CGFloat)0);
		color = [UIColor colorWithHue:h saturation:s brightness:b alpha:a];
	}
