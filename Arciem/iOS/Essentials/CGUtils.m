...
CGGradientRef GradientCreateEaseOut(CGColorRef color1, CGColorRef color2, CGColorSpaceRef colorSpace, NSUInteger steps)
{
    ...
    for(NSUInteger step = 0; step < steps; step++) {
        CGFloat t = normalize((CGFloat)step, (CGFloat)0.0, (CGFloat)(steps - 1));
        CGFloat phi = powf(ease_out(t), 0.5);
...
CGGradientRef GradientCreateSine(CGColorRef color1, CGColorRef color2, CGColorSpaceRef colorSpace, NSUInteger steps, CGFloat exponent)
{
    ...
    for(NSUInteger step = 0; step < steps; step++) {
        CGFloat t = normalize((CGFloat)step, (CGFloat)0.0, (CGFloat)(steps - 1));
        CGFloat phi = powf(fabsf(sinf(t * M_PI)), exponent);
//        CLogDebug(nil, @"step:%d t:%f phi:%f", step, t, phi);
        colorValues[step] = CreateColorByInterpolatingColors(color1, color2, phi);
    }
...
{
    NSUInteger kSteps = 8;
    CGColorRef *colorValues = malloc(sizeof(CGColorRef) * kSteps);
    for(NSUInteger step = 0; step < kSteps; step++) {
        CGFloat t = normalize((CGFloat)step, (CGFloat)0.0, (CGFloat)(kSteps - 1));
        CGFloat hue = fmodf(t + hueOffset, 1.0);
        if(hue < 0.0) hue += 1.0;
        CColorHSB *hsb = [CColorHSB newColorWithHue:hue saturation:saturation brightness:value alpha:alpha];
        UIColor *rgb = [hsb newUIColor];
...
CGColorRef CreateColorByDarkening(CGColorRef color, CGFloat fractionDarker)
{
	...
	switch(numberOfComponents) {
	case 2:
		newc[0] = denormalize(fractionDarker, (CGFloat)oldc[0], (CGFloat)0.0);
		newc[1] = newc[0];
		newc[2] = newc[0];
		newc[3] = oldc[1];
		break;
	case 4:
		newc[0] = denormalize(fractionDarker, (CGFloat)oldc[0], (CGFloat)0.0);
		newc[1] = denormalize(fractionDarker, (CGFloat)oldc[1], (CGFloat)0.0);
		newc[2] = denormalize(fractionDarker, (CGFloat)oldc[2], (CGFloat)0.0);
		newc[3] = oldc[3];
		break;
	}
...
CGColorRef CreateColorByLightening(CGColorRef color, CGFloat fractionLighter)
{
	...
	switch(numberOfComponents) {
	case 2:
		newc[0] = denormalize(fractionLighter, (CGFloat)oldc[0], (CGFloat)1.0);
		newc[1] = newc[0];
		newc[2] = newc[0];
		newc[3] = oldc[1];
		break;
	case 4:
		newc[0] = denormalize(fractionLighter, (CGFloat)oldc[0], (CGFloat)1.0);
		newc[1] = denormalize(fractionLighter, (CGFloat)oldc[1], (CGFloat)1.0);
		newc[2] = denormalize(fractionLighter, (CGFloat)oldc[2], (CGFloat)1.0);
		newc[3] = oldc[3];
		break;
	}
...
CGColorRef CreateColorByInterpolatingColors(CGColorRef color1, CGColorRef color2, CGFloat fraction)
{
	...
	if(CGColorGetNumberOfComponents(color1) == 4 && CGColorGetNumberOfComponents(color2) == 4) {
		...
		CGFloat newc[4];
		newc[0] = denormalize(fraction, c1[0], c2[0]);
		newc[1] = denormalize(fraction, c1[1], c2[1]);
		newc[2] = denormalize(fraction, c1[2], c2[2]);
		newc[3] = denormalize(fraction, c1[3], c2[3]);
