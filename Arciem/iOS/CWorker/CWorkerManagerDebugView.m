...
- (void)beginObservingWorkerManager
{
	...
	[self.workerManager addObserver:self forKeyPath:@"workers" options:(NSKeyValueObservingOptions)(NSKeyValueObservingOptionOld | NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial) context:nil];
}
...
- (void)beginObservingWorkers:(NSSet*)workers
{
	for(CWorker* worker in workers) {
		[worker addObserver:self forKeyPath:@"ready" options:(NSKeyValueObservingOptions)0 context:(void*)0x123];
		[worker addObserver:self forKeyPath:@"active" options:(NSKeyValueObservingOptions)0 context:(void*)0x123];
...
{
	[UIView animateWithDuration:kRemovalFadeAnimationDuration delay:0.0 options:(UIViewAnimationOptions)(UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseIn | UIViewAnimationOptionOverrideInheritedCurve) animations:^ {
		view.alpha = 0.0;
...
- (void)animatedLayout
{
	...
	[UIView animateWithDuration:kLayoutAnimationDuration delay:0.0 options:(UIViewAnimationOptions)(UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionCurveEaseOut) animations:^{
        [self layoutIfNeeded];
		for(CWorkerDebugView* view in self.queuedWorkerViews) {
			if(view.alpha != 1.0) {
				view.alpha = 1.0;
