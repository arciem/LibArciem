...
int xtime_get(struct xtime* xtp, int clock_type)
{
    if (clock_type == TIME_UTC)
    {
        ...
        const arciem::uint64_t TIMESPEC_TO_FILETIME_OFFSET =
            ((arciem::uint64_t)27111902UL << 32) +
            (arciem::uint64_t)3577643008UL;
...
{
	return static_cast<double>(sec) + static_cast<double>(nsec) / arciem::nanoseconds_per_second;
}
